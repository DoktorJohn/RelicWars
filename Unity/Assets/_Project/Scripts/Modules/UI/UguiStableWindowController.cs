...
{
    public sealed class UguiStableWindowController : MonoBehaviour
    {
        ...
        private StableUnitInfoDTO _selected;
        ...
        private void OnEnable()
        {
            ...
            if (state != null)
            {
                ...
                OnQueueChanged(state.CurrentStableQueue);
            }
        ...
        private void OnDisable()
        {
            ...
            if (CityStateManager.Instance != null)
            {
                ...
                CityStateManager.Instance.OnStableQueueChanged -= OnQueueChanged;
            }
            ClearQueueCards();
        }
        ...
        {
            string[] names = { "LightCavalry tab", "Knight tab", "Cataphracts tab" };
        ...
        private void LoadOverview()
        {
            ...
            if (network == null || network.Stable == null || !network.ActiveCityId.HasValue) return;
            StartCoroutine(network.Stable.GetStableOverviewInformation(network.ActiveCityId.Value, network.JwtToken, overview =>
            {
        ...
        private void BindOverview(List<StableUnitInfoDTO> units)
        {
        ...
        private void OnRecruitClicked(CarvedPressButton _)
        {
            ...
            NetworkManager network = NetworkManager.Instance;
            if (_selected == null || amount <= 0 || network?.Stable == null || !network.ActiveCityId.HasValue) return;
            StartCoroutine(network.Stable.RecruitUnits(network.ActiveCityId.Value, _selected.UnitType, amount, network.JwtToken, result =>
            {
        ...
        {
            NetworkManager network = NetworkManager.Instance;
            if (network?.Stable == null || !network.ActiveCityId.HasValue) return;
            button.enabled = false;
            StartCoroutine(network.Stable.CancelRecruitment(network.ActiveCityId.Value, queueId, network.JwtToken, result =>
            {
