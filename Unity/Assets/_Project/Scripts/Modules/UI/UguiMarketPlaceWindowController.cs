...
{
    public sealed class UguiMarketPlaceWindowController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            _rowTemplate = FindTransform(transform, "MarketPlaceBuildingLevelDataRow")?.gameObject;
        ...
        private void LoadMarketPlaceProjection()
        {
            int version = ++_requestVersion;
            NetworkManager network = NetworkManager.Instance;
            if (network == null || network.MarketPlace == null || !network.ActiveCityId.HasValue)
            {
                ShowEmptyState();
                return;
            }
            ...
            StartCoroutine(network.MarketPlace.GetMarketPlaceInfo(cityId, network.JwtToken, dataList =>
            {
        ...
        private void Render(IEnumerable<MarketPlaceInfoDTO> dataList)
        {
            List<MarketPlaceInfoDTO> levels = dataList
                .OrderBy(item => item.Level)
                .ToList();

            MarketPlaceInfoDTO currentLevel = levels.Find(item => item.IsCurrentLevel);
            SetText(_currentLevelLabel, currentLevel != null ? $"LEVEL {currentLevel.Level}" : "-");

            ClearRows();
            if (_rowsRoot == null || _rowTemplate == null) return;

            foreach (MarketPlaceInfoDTO level in levels)
            {
        ...
        private static void BindRow(Transform row, MarketPlaceInfoDTO level)
        {
            ...
            SetText(bonusText, $"+{level.ModifierIncrease * 100:N0}%");
