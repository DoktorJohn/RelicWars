...
{
    public sealed class UguiHousingWindowController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            _rowTemplate = FindTransform(transform, "HousingBuildingLevelDataRow")?.gameObject;
        ...
        private void LoadHousingProjection()
        {
            ...
            StartCoroutine(network.Building.GetHousingProjection(cityId, network.JwtToken, dataList =>
            {
        ...
        private void Render(IEnumerable<HousingInfoDTO> dataList)
        {
            List<HousingInfoDTO> levels = dataList
                .OrderBy(item => item.Level)
                .ToList();

            HousingInfoDTO currentLevel = levels.Find(item => item.IsCurrentLevel);
            SetText(_currentLevelLabel, currentLevel != null ? $"LEVEL {currentLevel.Level}" : "-");

            ClearRows();
            if (_rowsRoot == null || _rowTemplate == null) return;

            foreach (HousingInfoDTO level in levels)
            {
        ...
        private static void BindRow(Transform row, HousingInfoDTO level)
        {
            TMP_Text levelText = FindComponent<TMP_Text>(row, "LevelText");
            TMP_Text populationText = FindComponent<TMP_Text>(row, "PopulationText");
            SetText(levelText, level.Level.ToString("N0"));
            SetText(populationText, $"{level.Population:N0} total");

            SetBold(levelText, level.IsCurrentLevel);
            SetBold(populationText, level.IsCurrentLevel);
