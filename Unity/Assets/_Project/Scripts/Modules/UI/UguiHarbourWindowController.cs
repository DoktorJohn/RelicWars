...
{
    public sealed class UguiHarbourWindowController : MonoBehaviour
    {
        ...
        private HarborUnitInfoDTO _selected;
        ...
        private void OnEnable()
        {
            ...
            if (state != null)
            {
                ...
                OnQueueChanged(state.CurrentHarborQueue);
            }
        ...
        private void OnDisable()
        {
            ...
            if (CityStateManager.Instance != null)
            {
                ...
                CityStateManager.Instance.OnHarborQueueChanged -= OnQueueChanged;
            }
            ClearQueueCards();
        }
        ...
        {
            string[] names = { "Militia tab", "MenAtArms tab", "Spearmen tab", "Axemen tab", "Swordmen tab" };
            foreach (string name in names)
            {
                ...
                {
                    Button = button,
                    Name = root.GetComponentsInChildren<TMP_Text>(true)
                        .FirstOrDefault(text => !text.transform.name.Equals("OwnedAmount", StringComparison.Ordinal)),
        ...
        private void LoadOverview()
        {
            ...
            if (network == null || network.Harbor == null || !network.ActiveCityId.HasValue) return;
            StartCoroutine(network.Harbor.GetHarborOverviewInformation(network.ActiveCityId.Value, network.JwtToken, overview =>
            {
        ...
        private void BindOverview(List<HarborUnitInfoDTO> units)
        {
            for (int index = 0; index < _tabs.Count; index++)
            {
                TabBinding tab = _tabs[index];
                tab.Unit = index < units.Count ? units[index] : null;
                tab.Button.gameObject.SetActive(tab.Unit != null);
                if (tab.Unit == null) continue;
                // Bind the authored child labels instead of reaching into the tab package's private label field.
                SetText(tab.Name, Humanize(tab.Unit.UnitName).ToUpperInvariant());
        ...
        private void OnRecruitClicked(CarvedPressButton _)
        {
            ...
            NetworkManager network = NetworkManager.Instance;
            if (_selected == null || amount <= 0 || network?.Harbor == null || !network.ActiveCityId.HasValue) return;
            StartCoroutine(network.Harbor.RecruitUnits(network.ActiveCityId.Value, _selected.UnitType, amount, network.JwtToken, result =>
            {
        ...
        {
            NetworkManager network = NetworkManager.Instance;
            if (network?.Harbor == null || !network.ActiveCityId.HasValue) return;
            button.enabled = false;
            StartCoroutine(network.Harbor.CancelRecruitment(network.ActiveCityId.Value, queueId, network.JwtToken, result =>
            {
