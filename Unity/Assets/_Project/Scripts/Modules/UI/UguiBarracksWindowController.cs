...
{
    public sealed class UguiBarracksWindowController : MonoBehaviour
    {
        private sealed class TabBinding
        {
            ...
            public Image Icon;
            public BarracksUnitInfoDTO Unit;
        }
        ...
        private BarracksUnitInfoDTO _selected;
        ...
        private void OnEnable()
        {
            ...
            if (state != null)
            {
                ...
                OnQueueChanged(state.CurrentBarracksQueue);
            }
        ...
        private void OnDisable()
        {
            ...
            if (CityStateManager.Instance != null)
            {
                ...
                CityStateManager.Instance.OnBarracksQueueChanged -= OnQueueChanged;
            }
            ClearQueueCards();
        }
        ...
        private void LoadOverview()
        {
            ...
            if (network == null || network.Barracks == null || !network.ActiveCityId.HasValue) return;
            StartCoroutine(network.Barracks.GetBarracksOverviewInformation(network.ActiveCityId.Value, network.JwtToken, overview =>
            {
        ...
        private void OnRecruitClicked(CarvedPressButton _)
        {
            ...
            NetworkManager network = NetworkManager.Instance;
            if (_selected == null || amount <= 0 || network?.Barracks == null || !network.ActiveCityId.HasValue) return;
            StartCoroutine(network.Barracks.RecruitUnits(network.ActiveCityId.Value, _selected.UnitType, amount, network.JwtToken, result =>
            {
        ...
        {
            NetworkManager network = NetworkManager.Instance;
            if (network?.Barracks == null || !network.ActiveCityId.HasValue) return;
            button.enabled = false;
            StartCoroutine(network.Barracks.CancelRecruitment(network.ActiveCityId.Value, queueId, network.JwtToken, result =>
            {
