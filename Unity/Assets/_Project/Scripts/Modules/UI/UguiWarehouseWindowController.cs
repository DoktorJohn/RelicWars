...
{
    public sealed class UguiWarehouseWindowController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            _rowTemplate = FindTransform(transform, "WarehouseBuildingLevelDataRow")?.gameObject;
        ...
        private void LoadWarehouseProjection()
        {
            ...
            StartCoroutine(network.Building.GetWarehouseProjection(cityId, network.JwtToken, dataList =>
            {
        ...
        private void Render(IEnumerable<WarehouseProjectionDTO> dataList)
        {
            List<WarehouseProjectionDTO> levels = dataList
                .OrderBy(item => item.Level)
                .ToList();

            WarehouseProjectionDTO currentLevel = levels.Find(item => item.IsCurrentLevel);
            SetText(_currentLevelLabel, currentLevel != null ? $"LEVEL {currentLevel.Level}" : "-");

            ClearRows();
            if (_rowsRoot == null || _rowTemplate == null) return;

            foreach (WarehouseProjectionDTO level in levels)
            {
        ...
        private static void BindRow(Transform row, WarehouseProjectionDTO level)
        {
            TMP_Text levelText = FindComponent<TMP_Text>(row, "LevelText");
            TMP_Text capacityText = FindComponent<TMP_Text>(row, "CapacityText");
            SetText(levelText, level.Level.ToString("N0"));
            SetText(capacityText, level.Capacity.ToString("N0"));

            SetBold(levelText, level.IsCurrentLevel);
            SetBold(capacityText, level.IsCurrentLevel);
