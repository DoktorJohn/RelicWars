...
{
    public sealed class UguiTimberCampWindowController : MonoBehaviour
    {
        private TMP_Text _currentLevelLabel;
        private TMP_Text _amountHeaderLabel;
        ...
        private void Awake()
        {
            ...
            Transform amountHeader = FindTransform(transform, "AmountHeader");
            _amountHeaderLabel = amountHeader != null
                ? amountHeader.GetComponentInChildren<TMP_Text>(true)
                : null;
            _rowTemplate = FindTransform(transform, "TimberCampBuildingLevelDataRow")?.gameObject;
            _rowsRoot = _rowTemplate != null ? _rowTemplate.transform.parent : null;

            SetText(_amountHeaderLabel, "WOOD / HOUR");
