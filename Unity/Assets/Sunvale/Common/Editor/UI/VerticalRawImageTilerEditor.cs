...
{
    [CustomEditor(typeof(VerticalRawImageTiler))]
    public class VerticalRawImageTilerEditor : UnityEditor.Editor
    {
        private Texture2D packIcon;

        private const string Description =
            "When using a normal Unity Image with tiling, for example with a vertically tiling texture, changing the width also causes the image to tile horizontally.\n\n" +
            "This component solves that problem: it tiles vertically while still allowing the image to stretch horizontally like a normal Image.";
